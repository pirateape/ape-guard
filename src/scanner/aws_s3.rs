...
mod tests {
    ...
    fn test_parse_output_manual_inspection() {
        ...
        assert!(result.is_ok());
        let findings = result.expect("aws_s3 test: scan should succeed");
    ...
    fn test_parse_output_empty_input() {
        ...
        assert!(result.is_ok());
        let findings = result.expect("aws_s3 test: empty parse should succeed");
        assert!(findings.is_empty());
    }
    ...
    fn test_parse_output_ok_response() {
        let scanner = AwsS3Scanner::new("test.json");
        let raw = b"OK: No public S3 buckets or encryption issues detected";
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("aws_s3 test: OK parse should succeed");
        assert!(findings.is_empty());
    }
    ...
    fn test_parse_output_aws_cli_json() {
        let scanner = AwsS3Scanner::new("test.json");
        let raw = br#"{"Buckets":[{"Name":"my-bucket","LastModified":"2026-01-01"}]}"#;
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("aws_s3 test: JSON parse should succeed");
        assert!(!findings.is_empty());
        assert_eq!(findings[0].rule_id, "AWS001");
    }

    #[test]
    fn test_parse_output_no_relevant_lines() {
        let scanner = AwsS3Scanner::new("test.json");
        let raw = b"some random log line\nanother line\n";
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("aws_s3 test: irrelevant parse should succeed");
        assert!(findings.is_empty());
    }
}
