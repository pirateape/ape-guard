    ...
    fn parse_output(&self, raw: &[u8]) -> Result<Vec<CanonicalFinding>, ScannerError> {
        ...
        if output.contains("notBefore") || output.contains("notAfter") {
            ...
            for (i, line) in output.lines().enumerate() {
                let (sev, title) = if line.contains("EXPIRING") || line.contains("EXPIRED") {
                    (Severity::Critical, "Certificate expired or expiring")
                } else if line.contains("CERT FOUND") {
                    (Severity::Medium, "Certificate requires verification")
                } else if line.contains("NOT FOUND") {
                    (Severity::Low, "Certificate file not found")
                } else {
                    continue;
                };

                let detail = line.trim_start_matches("⚠️ ").trim();
...
mod tests {
    ...
    fn test_parse_output_manual_inspection() {
        ...
        assert!(result.is_ok());
        let findings = result.expect("tls test: scan should succeed");
    ...
    fn test_parse_output_empty_input() {
        ...
        assert!(result.is_ok());
        let findings = result.expect("tls test: empty parse should succeed");
        assert!(findings.is_empty());
    }

    #[test]
    fn test_parse_output_expired_path() {
        let scanner = TlsScanner::new(&["/etc/ssl/certs/cert.pem"]);
        let raw = b"EXPIRED: cert.pem";
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("tls test: EXPIRED parse should succeed");
        assert_eq!(findings.len(), 1);
        assert_eq!(findings[0].rule_id, "TLS001");
    }

    #[test]
    fn test_parse_output_cert_found_path() {
        let scanner = TlsScanner::new(&["/etc/ssl/certs/cert.pem"]);
        let raw = b"\xe2\x9a\xa0\xef\xb8\x8f CERT FOUND: cert.pem (requires openssl to verify)";
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("tls test: CERT FOUND parse should succeed");
        assert_eq!(findings.len(), 1);
        assert_eq!(findings[0].rule_id, "TLS002");
    }

    #[test]
    fn test_parse_output_not_found_path() {
        let scanner = TlsScanner::new(&["/etc/ssl/certs/cert.pem"]);
        let raw = b"NOT FOUND: missing.pem";
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("tls test: NOT FOUND parse should succeed");
        assert_eq!(findings.len(), 1);
        assert_eq!(findings[0].rule_id, "TLS003");
    }

    #[test]
    fn test_parse_output_ok_response() {
        let scanner = TlsScanner::new(&["/etc/ssl/certs/cert.pem"]);
        let raw = b"OK: No TLS certificates scanned";
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("tls test: OK parse should succeed");
        assert!(findings.is_empty());
    }
    ...
    fn test_parse_output_no_relevant_lines() {
        ...
        let result = scanner.parse_output(raw);
        assert!(result.is_ok());
        let findings = result.expect("tls test: irrelevant parse should succeed");
        assert!(findings.is_empty());
    }
}
